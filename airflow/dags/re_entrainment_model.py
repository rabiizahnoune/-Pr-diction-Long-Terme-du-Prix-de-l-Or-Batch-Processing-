...
def prepare_data_for_lstm(**context):
    ...
    # Connexion à Cassandra pour récupérer les données
    cluster = Cluster(['cassandra'], port=9042)
    session = cluster.connect('crypto')
    rows = session.execute("SELECT * FROM gold_transformed ORDER BY datetime")
    df = pd.DataFrame(list(rows))
    cluster.shutdown()

    # Normaliser uniquement la colonne close_price (comme dans ton code)
    scaler = MinMaxScaler()
    ...
    context['ti'].xcom_push(key='scaler', value=scaler)  # Sauvegarder le scaler pour inverser la normalisation
    print("Données préparées pour le LSTM.")
...
def train_lstm(**context):
    ...
    scaler = context['ti'].xcom_pull(key='scaler')

    # Reshape des données pour LSTM (ajouter une dimension pour les features)
...
with DAG(
    ...
    schedule_interval='0 0 * * 0',  # Exécuter tous les dimanches à minuit
    catchup=False
) as dag:
