...
def fetch_gold_data_monthly(**context):
    """Récupère les données horaires de l'or (GC=F) pour un mois depuis yfinance."""
    symbol = 'GC=F'
    end_time = datetime.now()
    start_time = end_time - timedelta(days=7)  # 30 jours pour un mois approx.
    ...
    try:
        ...
        df = gold.history(
            ...
            interval='1m'  # Intervalle de 1 heure
        )
...
with DAG(
    ...
    fetch_data = PythonOperator(
        task_id='fetch_data',
        python_callable=fetch_gold_data_monthly,
